...
{
    public static class ToChucMuaBanService
    {
        public static DataTable GetAll()
        {
            return DatabaseConnection.ExecuteQuery("SELECT * FROM ToChucMuaBan WHERE IsActive=1 ORDER BY ID DESC");
        }

        public static bool Add(ToChucMuaBan t)
        {
            var sql = @"INSERT INTO ToChucMuaBan (TenToChuc,LoaiHinh,LoaiSanPham,DiaChi,SoDienThoai,Email,MaSoThue,GiayPhepKinhDoanh,GhiChu)
                        VALUES (@Ten,@LoaiHinh,@LoaiSP,@DiaChi,@SDT,@Email,@MST,@GPKD,@GhiChu)";
            return DatabaseConnection.ExecuteNonQuery(sql, Params(t)) > 0;
        }

        public static bool Update(ToChucMuaBan t)
        {
            var sql = @"UPDATE ToChucMuaBan SET TenToChuc=@Ten,LoaiHinh=@LoaiHinh,LoaiSanPham=@LoaiSP,
        ...
        public static bool Delete(int id)
        {
            return DatabaseConnection.ExecuteNonQuery(
                "UPDATE ToChucMuaBan SET IsActive=0 WHERE ID=@id",
        ...
        public static DataTable Search(string keyword)
        {
            return DatabaseConnection.ExecuteQuery(
                "SELECT * FROM ToChucMuaBan WHERE IsActive=1 AND (TenToChuc LIKE @kw OR DiaChi LIKE @kw OR SoDienThoai LIKE @kw) ORDER BY ID DESC",
                new[] { new SqlParameter("@kw", $"%{keyword}%") });
        }

        private static SqlParameter[] Params(ToChucMuaBan t) => new[]
        {
